...
for data in data_toko:
    ...
    connect_toko.commit()
    if jumlah == 0:
        insert = "INSERT INTO tb_integrasi(id_invoice, total_transaksi, status) values(%s,%s,%s)" % (
        id_invoice, total_transaksi, status)
        cursor_toko.execute(insert)
        cursor_bank.execute(insert)
        connect_toko.commit()
        connect_bank.commit()

        insert_bank = "INSERT INTO tb_invoice(id_invoice, total_transaksi, status) values(%s,%s,%s)" % (
        id_invoice, total_transaksi, status)
        cursor_bank.execute(insert_bank)
        connect_bank.commit()
        print(
            "TERDAPAT PENAMBAHAN DATA PADA db_bank.tb_invoice. Pada id_invoice = %s, total_transaksi = %s, status = %s" % (
            id_invoice, total_transaksi, status))
